...
export async function POST(req: Request) {
  ...
  const { data: user } = await supabase.from("users_data").select("free_chats_remaining, subscription_status").eq("id", userId).single();
  ...
  await supabase.from("users_data").update({ free_chats_remaining: remaining }).eq("id", userId);
